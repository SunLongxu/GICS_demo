...
class SubGraph(object):
    def __init__(self, args, graph, features, target, authorname, keywords):
        ...
        self.oldres = []
        self.allnode = []
        self.cmty_siz = args.community_size
    ...
    def build_local_candidate_iteration(self, seed, round):
        ...
        if (round > 0):
            # 不导入ICSGNNWrapper，直接检查self对象
            if hasattr(self, 'recommender') and self.recommender is not None:
                # 使用已存在的recommender对象
                rec = self.recommender
                print(f"使用SubGraph对象中的recommender，不再设置回调")
            else:
                # 如果没有现有对象，才创建新的实例
                rec = PPRRecommend(self.args, self)
                print("创建新的recommender实例")
                # 保存到self以便下次使用
                self.recommender = rec
                
                # 尝试从全局实例获取回调并设置到新创建的recommender
                try:
                    ...
                    if hasattr(icsgnn_instance, 'state_callback') and icsgnn_instance.state_callback is not None:
                        print(f"为新创建的recommender设置全局状态回调")
                        rec.set_callback(icsgnn_instance.state_callback)
                        print(f"全局回调设置结果: {rec.callback is not None}")
                except Exception as e:
                    print(f"尝试设置全局回调失败: {str(e)}")
